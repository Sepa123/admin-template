...
import { VentaOTraspasoService } from 'src/app/service/venta-o-traspaso.service';
@Component({
...
export class VentaOTraspasoDeVehiculoComponent {
  ...
  BuscarRazon(event: Event) {
    ...
    this.RazonesFiltradas = this.infoEmpresa2.filter((patente) => {
      return (
        patente.rut?.toString().includes(valorBusqueda) ||
        patente.razon_social?.toLowerCase().includes(valorBusqueda)
        // patente.tipo?.toString().includes(valorBusqueda) ||
        // patente.colaborador_id?.toString().includes(valorBusqueda) ||
        // patente.tripulacion?.toLowerCase().includes(valorBusqueda)
  ...
  CambiarRazonSocial() {
    if (
      this.DatoPatenteSeleccionados !== null &&
      this.DatoEmpresaNueva !== null
    ) {
  ...
  obtenerDatosIniciales(
    ...
    console.log(
      this.DatoPatenteSeleccionados,
      this.DatoEmpresaActual,
      this.DatoIdEmpresaActual,
      this.DatoRutEmpresa,
      this.DatoPpuSeleccionada
    );
  }
  ObtenerDatosResultantes(
    IdEmpresaNueva: any,
    RutEmpresaNueva: any,
    nombreEmpresa: any
  ) {
