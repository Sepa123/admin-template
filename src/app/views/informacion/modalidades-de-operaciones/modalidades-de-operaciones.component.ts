...
export class ModalidadesDeOperacionesComponent implements OnInit, OnDestroy {
  ...
  submitForm() {
    ...
    this.http
      .post('http://localhost:8000/Agregar/RazonSocial', formData)
      .subscribe((data) => {
  ...
  buscarDatos() {
    const url = `http://localhost:8000/Api/Modalidad?search=${this.searchTerm}`;
  ...
  eliminarRazonSocial(id: number) {
    ...
    this.http
      .delete(`http://localhost:8000/Modalidad/borrar?id=${id}`)
      .subscribe(
        () => {
